...
module.exports.run = async ({ api, event, args, getText }) => {
	...
	var we = "9de32e8f21dea5dcc515bbb3c26f5d42"
	var city = args.join(" ");
	if (city.length == 0) return throwError(this.config.name, threadID, messageID);
	return request(encodeURI("https://api.openweathermap.org/data/2.5/weather?q=" + city + `&appid=${we}`  + "&units=metric&lang=" + global.config.language), (err, response, body) => {
		if (err) throw err;
